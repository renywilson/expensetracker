...
inputElement.style.height = "25px";
  ...
  const userleaderboardArray= await axios.get('http://35.175.246.186:4500/premium/showLeaderBoard',{headers:{"Authorization":token}})
...
document.addEventListener("DOMContentLoaded",(event) =>{
 ...
 if(isadmin)
 {
  ...
  showprebutton();
    ...
    axios.get(`http://35.175.246.186:4500/expense/getexpenses?page=${page}`,{headers:{"Authorization":token}})
...
function onsubmit(e){
    ...
    console.log(obj);
    const token=localStorage.getItem('token')
    axios.post("http://35.175.246.186:4500/expense/addexpense",obj,{headers:{"Authorization":token}})
    .then((res) =>{
...
function createElement(objun, currentPage, itemsPerPage) {
  ...
  deletebutton.addEventListener("click", () => {
     const token=localStorage.getItem('token')
       ...
       axios
        .delete(`http://35.175.246.186:4500/expense/deleteexpense/${objun.id}`,{headers:{"Authorization":token}})
        .then((res) =>
        console.log(res.data),
         li.remove(),
...
dropdown.addEventListener('change', (event) => {
  ...
  const token = localStorage.getItem('token');
  const url = `http://35.175.246.186:4500/expense/getexpenses?pageSize=${selectedPageSize}`;
...
function getExpenses(page) {
  ...
  console.log(selectedPageSize)
  axios.get(`http://35.175.246.186:4500/expense/getexpenses?page=${page}&pageSize=${selectedPageSize} `,{headers:{"Authorization":token}})
  .then(res =>{ 
...
document.getElementById('rzp-button1').onclick=async function(e){
 ...
 const response= await axios.get('http://35.175.246.186:4500/purchase/premiummembership',{headers:{"Authorization":token}});
  console.log(response);
   var options=
   {
    ...
    const res= await axios.post('http://35.175.246.186:4500/purchase/updatetransactionstatus',{order_id:options.order_id,payment_id:response.razorpay_payment_id,}, {headers:{"Authorization":token}});
...
toggleButton.addEventListener('click', async () => {
 ...
 const response= await axios.get('http://35.175.246.186:4500/expense/downloadfile',{headers:{"Authorization":token}});
...
function download(){
  const token=localStorage.getItem('token')
  axios.get('http://35.175.246.186:4500/expense/download', { headers: {"Authorization" : token} })
  .then((response) => {
